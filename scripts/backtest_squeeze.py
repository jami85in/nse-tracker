...
TARGET_PCT   = 0.03    # GTT limit-sell target (changed from 4% per user request)
STOP_PCT     = 0.05    # GTT market stop-loss — unchanged
...
def simulate_stock(symbol, raw, entry_after=None):
    ...
    while i < n - 2:  # need bar i+1 to exist for the next-open entry
        if (not np.isnan(bbw[i]) and not np.isnan(sk[i]) and not np.isnan(sd[i])
            ...
            exit_slip = SLIPPAGE_PCT  # applied to market-type exits (stop / timeout)
            for j in range(i + 1, last_j + 1):
                # Stop checked first (conservative). A GTT stop-loss fires as a
                # MARKET order, so gap-through fills at the open (worse) and
                # slippage applies.
                if lows[j] <= stp:
                    exit_raw = min(stp, opens[j]); result = "STOP"; exit_idx = j; break
                # A GTT target is a LIMIT sell — you get filled AT the target
                # (or better on a gap up), with no adverse slippage.
                if highs[j] >= tgt:
                    exit_raw = max(tgt, opens[j]); result = "TARGET"; exit_idx = j
                    exit_slip = 0.0
                    break
            if result == "TIMEOUT":
                exit_idx = last_j
            ...
            exit_fill = exit_raw * (1 - exit_slip)              # limit exit -> no slip; market -> slip
